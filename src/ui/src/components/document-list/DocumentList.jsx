...
const DocumentList = () => {
  ...
  const renderExpandedTransactionContent = (item) => {
    const rawData = item.rawData || {};
    const isAnalyzed = item.analysisStatus === 'ANALYZED';
      ...
      <Box padding="l" variant="div">
        <SpaceBetween size="l">
          ...
          <Container header={<Header variant="h3">Transaction Details</Header>}>
            <ColumnLayout columns={3} variant="text-grid">
              ...
              <div>
                ...
                <Badge>{rawData.TransactionType || item.transactionType || 'N/A'}</Badge>
              </div>
              <div>
                <Box variant="awsui-key-label">Source Page</Box>
                <Box>{rawData.SourcePage || 'N/A'}</Box>
              </div>
            </ColumnLayout>
          </Container>
                ...
                <ColumnLayout columns={3} variant="text-grid">
                  ...
                  <div>
                    ...
                    <StatusIndicator type={rawData.HMRCConcern ? 'warning' : 'success'}>
                      {rawData.HMRCConcern ? 'Requires attention' : 'No concerns'}
                    </StatusIndicator>
                  </div>
                </ColumnLayout>
          ...
          {isAnalyzed && rawData.RiskFlags && rawData.RiskFlags.length > 0 && rawData.RiskFlags[0] !== 'CLEAN' && (
            <Container header={<Header variant="h3">Risk Details</Header>}>
              <SpaceBetween size="m">
                <div>
                ...
                <ColumnLayout columns={2} variant="text-grid">
                  <div>
                    ...
                    <Badge color={rawData.ComplianceRiskTier === 'HIGH' ? 'red' : 'grey'}>
      ...
      <Table
        ...
        expandableRows={{
          getItemChildren: (item) => {
            ...
            return [{ content: renderExpandedTransactionContent(item) }];
          },
          isItemExpandable: () => true,
          expandedItems: Array.from(expandedItems).map((id) => bankStatementItems.find((t) => t.id === id)),
